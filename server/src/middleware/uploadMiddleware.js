...
const processUpload = (uploader, req, res) =>
  new Promise((resolve, reject) => {
    ...
    uploader.array(fieldName, maxCount)(req, res, err => {
      ...
      // Initialize empty array if no files were uploaded
      req.files = req.files || [];
      resolve();
    });
  });
...
// Main upload handler
const handleUpload = async (uploader, uploadFolder, req, res, next) => {
  try {
    ...
    // Map the uploaded files to the request body for validation
    ...
    if (req.uploadedFiles && req.uploadedFiles.length > 0) {
      await Promise.all(
        req.uploadedFiles.map(file =>
          cloudinary.uploader.destroy(file.publicId).catch(error => {
