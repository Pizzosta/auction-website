...
const createRateLimiter = ({
  ...
  if (RedisStoreCtor) {
    const sendCommand = async (...args) => {
      ...
      try {
        // Works for SCRIPT, EVALSHA, SET, GET, etc.
        return await client.sendCommand(args);
      } catch (err) {
        logger.error('Redis command failed:', { 
          error: err.message, 
    ...
    options.store = new RedisStoreCtor({ 
      sendCommand,
      prefix: 'rl:',
      // Custom script for atomic increment with expiry
      customScript: async (key, windowMs) => {
        const client = await getRedisClient();
        ...
        const [[, counter], [, ttl]] = results;
        
        // If key is new or has no expiry, set the expiry
        if (counter === 1 || ttl === -1) {
