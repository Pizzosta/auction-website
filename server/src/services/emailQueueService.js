...
// Track ioredis clients created by Bull so we can gracefully close them on shutdown
const bullClients = new Set();
...
const createClient = type => {
  ...
  // track so we can shutdown gracefully
  bullClients.add(client);
  return client;
};
...
export async function getEmailQueue() {
  ...
  emailQueue.process(async job => {
    ...
    try {
      await sendTemplateEmail(type, recipientEmail, {
        ...context,
        ...(typeof to === 'object' ? to : {}),
      });
...
async function monitorQueueHealth() {
  if (!emailQueue)
    throw new AppError('EMAIL_QUEUE_NOT_INITIALISED', 'Email queue not initialised', 500);
...
export async function closeQueues() {
  ...
  // ensure any ioredis clients we created are quit
  try {
    await Promise.all(Array.from(bullClients).map(c => c.quit().catch(() => {})));
  } catch (e) {
